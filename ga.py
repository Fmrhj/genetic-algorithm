...
class Individual:
    def __init__(self, params, value=None):
        if value is None:
            self.value = [np.random.uniform(params['lower_bound'], params['upper_bound'], 1)[
                0] for i in range(params['number_of_genes'])]
        else:
            self.value = value
...
class Population:
    def __init__(self, individual_class, pop_parameters, ind_parameters, fitness_function, ind_values=None):
        self.individual_class = individual_class
        if ind_values is None:
            self.individuals = [individual_class(ind_parameters) for _ in range(pop_parameters['size'])]
        else:
            self.individuals = [individual_class(ind_parameters, value) for value in ind_values]
    ...
    def get_parents(self, n_parents):
        ...
        parents.sort(key=lambda x: self.fitness_function(
...
class Evolution:
    def __init__(self, fitness, population_class, individual_class, pop_parameters, ind_parameters, fitness_function):
        self.fitness = fitness_function
        self.population_class = population_class
        self.individual_class = individual_class
        self.population = population_class(
            individual_class, pop_parameters, ind_parameters, fitness_function)
    ...
    def step(self):
        ...
        population_values = [self.population.individuals[i].get_values()
                             for i in range(self.population.size)]
        population_values.extend(mutation)
        population_values.sort(key=lambda x: self.fitness(x[0], x[1]), reverse=True)
        population_values = population_values[:self.pop_parameters['size']]
        self._best_individual = population_values[:1]
        self._best_score = self.fitness(self._best_individual[0][0], self._best_individual[0][1])
        # update population
